...
public class CustomerService {
    ...
    public void SetCustomer() {
        this.saveList = repository.findAll().stream()
                .map(customer ->
                        new CustomerStats(
    ...
    public List<CustomerStats> byMoneySpent() {
        if (saveList==null){
            SetCustomer();
        }
    ...
    public List<CustomerStats> byWatchedMovies() {
        if (saveList==null){
            SetCustomer();
        }
        return saveList.stream()
    ...
    public List<RentalStats> rentMoviesByMonths(int year) {
        ...
        IntStream.rangeClosed(1, 12).forEach(i ->
                sumOfRentals.add(repository.findAll().stream().map(x -> x.getRentalsByMonth(year, i)).reduce(0, Integer::sum)));
        IntStream.rangeClosed(1, 12).forEach(i ->
                rentalStats.add(new RentalStats(i, sumOfRentals.get(i - 1))));
    ...
    public Object getMoviesForCustomer(int year, int id) {
        ...
        IntStream.rangeClosed(1, 12).forEach(i ->
                temp.add(repository.findById(id).stream().map(x -> x.getRentalsByMonth(year, i)).reduce(0, Integer::sum)));
        IntStream.rangeClosed(1, 12).forEach(i ->
                monthStats.add(new RentalStats(i, temp.get(i-1))));
        return monthStats;
    }
    ...
    public byte[] generateCustomerPieChart(String title, String field, List<CustomerStats> entryData) throws IOException {
        ...
        switch (field) {
            case "movies":
                entryData.forEach(customerStats ->
                        dataset.setValue(
            ...
            case "money":
                entryData.forEach(customerStats ->
                        dataset.setValue(customerStats.getCustomerId(),
