...
import com.example.demo.repositories.exceptions.FilmNotFoundException;
import com.example.demo.repositories.exceptions.NoSuchPageException;
...
public class FilmsRepository {
    ...
    public List<FilmDto> getFilmsByPage(int page, int size, FilmDto film) {
        ...
        try {
            return (List<FilmDto>) entityManager.createNativeQuery(query + " limit "
                            + size + " offset " + (page - 1) * size, Film.class).getResultStream().map(f -> createFilmDto((Film) f))
    ...
    public HttpStatus updateFilm(FilmDto film) {
        try {
            ...
            entityManager.createQuery("update Film f set " +
                    ...
                    .setParameter("rentalDuration", film.getRentalDuration())
