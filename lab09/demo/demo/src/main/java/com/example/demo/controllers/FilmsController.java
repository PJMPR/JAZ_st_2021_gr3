...
public class FilmsController {
    ...
    @GetMapping
    public ResponseEntity<List<FilmDto>> getFilms(@RequestParam(defaultValue = "0")  Integer page,
                                                  ...
                                                  @RequestParam(required = false) BigDecimal replacement_costs) throws Exception {
        FilmDto film = FilmDto.builder().id(id).title(title)
                .releaseYear(release_year).rentalDuration(rental_duration)
                .rentalRate(rental_rate).replacementCosts(replacement_costs)
    ...
    @DeleteMapping("{id}")
    public ResponseEntity<HttpStatus> deleteFilm(@PathVariable int id) throws Exception {
        return ResponseEntity.status(filmsRepository.deleteFilmById(id)).build();
    }

    @PutMapping("{id}")
    public ResponseEntity<HttpStatus> updateFilm(@PathVariable int id, @RequestBody FilmDto film) throws Exception {
